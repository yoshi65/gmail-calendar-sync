...
class FlightEmailProcessor(BaseEmailProcessor):
    ...
    def process(self, email: EmailMessage) -> ProcessingResult:
        ...
        logger.info("Processing flight email",
                   email_id=email.id,
        ...
        try:
            ...
            logger.error("Failed to process flight email",
                        email_id=email.id,
                        error=str(e))
    ...
    def extract_data(self, email: EmailMessage) -> dict | None:
        ...
        try:
            ...
            logger.error("Failed to extract flight data",
                        email_id=email.id,
                        error=str(e))
            return None
    ...
    def create_calendar_events(self, extracted_data: dict, email: EmailMessage) -> list[CalendarEvent]:
        ...
        try:
            ...
            if existing_events:
                logger.info("Found existing events, checking for updates",
                           email_id=email.id,
                           confirmation_code=confirmation_code,
                           booking_reference=booking_reference,
                ...
                for i, new_event in enumerate(new_calendar_events):
                    if i < len(existing_events):
                        ...
                        if self.calendar_client.needs_update(existing_event, new_event):
                            ...
                            success = self.calendar_client.update_event(event_id, new_event)
                            if success:
                                updated_count += 1
                                logger.info("Updated existing calendar event",
                                           event_id=event_id,
                                           confirmation_code=confirmation_code)
                            else:
            ...
            logger.error("Failed to create/update calendar events",
                        email_id=email.id,
                        error=str(e))
            return []
