...
class EmailFilter:
    ...
    def __init__(self):
        ...
        self.promotional_body_patterns: list[Pattern[str]] = [
            ...
            re.compile(r'(キャンペーン|プレゼント|抽選).*?(キャンペーン|プレゼント|抽選)', re.IGNORECASE),
        ]
    ...
    def is_promotional_email(self, email: EmailMessage) -> bool:
        ...
        if self._is_booking_confirmation(subject, body):
            logger.debug("Email identified as booking confirmation",
                        email_id=email.id,
                        subject=subject[:100])
            return False

        # Check for promotional patterns in subject
        promotional_subject_matches = sum(1 for pattern in self.promotional_subject_patterns
                                        if pattern.search(subject))

        # Check for promotional patterns in body
        promotional_body_matches = sum(1 for pattern in self.promotional_body_patterns
        ...
        if promotional_subject_matches >= 2:
            is_promotional = True
            logger.info("Email identified as promotional (multiple subject patterns)",
                       email_id=email.id,
                       subject=subject[:100],
        ...
        # Strong promotional pattern in subject + body evidence
            ...
            is_promotional = True
            logger.info("Email identified as promotional (subject + multiple body patterns)",
                       email_id=email.id,
                       subject=subject[:100],
                       subject_matches=promotional_subject_matches,
        ...
        # Very high volume of promotional content in body only
            ...
            is_promotional = True
            logger.info("Email identified as promotional (many body patterns)",
                       email_id=email.id,
                       subject=subject[:100],
                       body_matches=promotional_body_matches)

        if not is_promotional:
            logger.debug("Email passed promotional filter",
                        email_id=email.id,
                        subject=subject[:100],
                        subject_matches=promotional_subject_matches,
                        body_matches=promotional_body_matches)
    ...
    def _is_booking_confirmation(self, subject: str, body: str) -> bool:
        ...
        confirmation_matches = sum(1 for pattern in self.booking_confirmation_patterns
                                 if pattern.search(body))

        return confirmation_matches >= 2  # Require multiple confirmation indicators in body
